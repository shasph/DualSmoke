...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256, use_wandb=False):
    ...
    for label, im_data in visuals.items():
        ...
        if im.shape[2] == 2:
            ch_zeros = np.zeros((256, 256, 1), np.uint8)
            im = np.concatenate([im, ch_zeros], 2)
            im = im.astype(np.uint8)
        image_name = '%s_%s_%s.png' % (image_path[0].split('/')[3], name, label)
            ...
            if ncols > 0:        # show all the images in one visdom panel
                ...
                for label, image in visuals.items():
                    ...
                    if image_numpy.shape[2] == 2:
                        ch_zeros = np.zeros((256, 256, 1), int)
                        image_numpy = np.concatenate([image_numpy, ch_zeros], 2)
